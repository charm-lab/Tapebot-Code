...
void PIDControllerV::setpoint (double newSetpoint) {
  setPoint = newSetpoint;
}

void PIDControllerV::tune (double _Kp, double _Ki, double _Kd) {
...
void PIDControllerV::limit(double min, double max) {
  minOut = min;
  maxOut = max;
  doLimit = true;
}

void PIDControllerV::printGraph (double sensorInput, String verbose) {
...
void PIDControllerV::minimize (double newMinimize) {
  divisor = newMinimize;
}

// Getters
double PIDControllerV::getOutput () {
  return output;
}


double PIDControllerV::compute (double sensor, String graph, String verbose) {
  ...
  double newOutput = (oldOutput + Kp * (error-lastErr) + Ki * errSum + Kd * dErr) / 1;
